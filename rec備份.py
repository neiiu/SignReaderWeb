...
def pre():
    ...
    # actions = np.array(['', '','','','','' ])
    actions = np.array(["bellyache", 'catchACold', 'come', 'give', 'inspect', 'lookFor', 'winge'])
    actionsChinese = np.array(["腹痛", '感冒', '來', '給', '檢察', '找', 'winge'])
    ...
    model.load_weights('./static/model/health.h5')
    ...
    sentence = []
    threshold = 0.8 
    target_action = actions[3]  # 设置目标动作
    target_action_Chinese = actionsChinese[3]
    ...
    #             print(actions[np.argmax(res)])
                ...
                if frame_count >= target_frame_count:
                    ...
                    cv2.rectangle(image, (0, 0), (800, 40), (0, 255, 0), -1)
        ...
        video_path = os.path.join('static', 'videos', 'allVideos', f'{target_action_Chinese}.mp4')  
        # 替换成你视频的实际路径
